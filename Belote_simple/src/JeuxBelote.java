...
public class JeuxBelote {
	...
	public static void main (String [] arguments)
	{
		...
		for (int pmax=0; pmax<500;){
			...
			jeuxatout=retourne.getCouleur().getNom();
			tapis.affiche();
			//affiche le jeux des joueurs
			for (int y = 0 ; y<4 ;y++){
				joueur[y].affichejoueur(y);
			...
			// remet les cartes dans le paquet
				for (int i=0;i<4;i++){
					tapis.nbcartej[i]=jeux.remetjeux(joueur[i].getMonPaquet(),tapis.nbcartej[i]);
				}	
				...
				for (int y =0 ; y<8;y++){
					...
					while (n2!=4){
						...
						if (n2==0){
							jeux.setCartejoue(jeux.getTapisjeux()[n1].getCouleur().getNom());
						}
						tapis.nbcartej[n1]=tapis.nbcartej[n1]-1;
					...
					Terminal.ecrireStringln("le joueur "+gagne+" gagne le plis");
					//affiche les plis
					jeux.affiche2();
				}
				...
				if (jeux.getPosplis()[0]==0 || jeux.getPosplis()[1]==0){
					...
					if (manchej==true){
						tapis.pointj1j3=tapis.pointj1j3+jeux.getPointplis()[1];
						tapis.pointj0j2=tapis.pointj0j2+jeux.getPointplis()[0];
					}else{
						if (prend==1 || prend==3){
